...
class Map extends Camera {
  ...
  constructor(options: MapOptions) {
    ...
    if (
      options.minPitch != null &&
      options.maxPitch != null &&
    ...
    if (options.maxPitch != null && options.maxPitch > maxPitchThreshold) {
      throw new Error(
        `maxPitch must be less than or equal to ${maxPitchThreshold}`
      );
    }
    ...
    if (!this._hash || !this._hash._onHashChange()) {
      ...
      if (options.bounds) {
        ...
        this.fitBounds(
          options.bounds,
          extend({}, options.fitBoundsOptions, { duration: 0 })
        );
      }
    }
    ...
    if (options.attributionControl)
      this.addControl(
        new AttributionControl({ customAttribution: options.customAttribution })
      );
  ...
  _createDelegatedListener(
    ...
    if (type === "mouseenter" || type === "mouseover") {
      let mousein = false;
      const mousemove = (e) => {
        const features = this.getLayer(layerId)
          ? this.queryRenderedFeatures(e.point, { layers: [layerId] })
          : [];
        if (!features.length) {
          mousein = false;
        } else if (!mousein) {
          ...
          listener.call(
            this,
            new MapMouseEvent(type, this, e.originalEvent, { features })
          );
        }
      };
      const mouseout = () => {
  ...
  on(
    type: MapEvent,
    layerIdOrListener: string | Listener,
    listener?: Listener
  ): this {
  ...
  off(
    ...
    const removeDelegatedListener = (delegatedListeners) => {
      ...
      for (let i = 0; i < listeners.length; i++) {
        ...
        if (
          delegatedListener.layer === layerIdOrListener &&
  ...
  _diffStyle(
    ...
    if (typeof style === "string") {
      const url = style;
      const request = this._requestManager.transformRequest(
        url,
        ResourceType.Style
      );
  ...
  isSourceLoaded(id: string) {
    ...
    if (source === undefined) {
      this.fire(
        new ErrorEvent(new Error(`There is no source with ID '${id}'`))
      );
      return;
    }
  ...
  updateImage(
    ...
    const imageData =
      image instanceof HTMLImageElement || isImageBitmap(image)
    ...
    if (
      width !== existingImage.data.width ||
      height !== existingImage.data.height
    ) {
      return this.fire(
        new ErrorEvent(
          new Error(
  ...
  loadImage(url: string, callback: Callback<HTMLImageElement | ImageBitmap>) {
    getImage(
      this._requestManager.transformRequest(url, ResourceType.Image),
      callback
    );
  }
  ...
  _setupContainer() {
    ...
    const controlContainer = (this._controlContainer = DOM.create(
      "div",
    ...
    ["top-left", "top-right", "bottom-left", "bottom-right"].forEach(
      (positionName) => {
        positions[positionName] = DOM.create(
          "div",
          `maplibregl-ctrl-${positionName} mapboxgl-ctrl-${positionName}`,
          controlContainer
        );
      }
    );
  ...
  _render(paintStartTimeStamp: number) {
    ...
    if (this.listens("gpu-timing-frame")) {
      ...
      setTimeout(() => {
        const renderGPUTime =
          extTimerQuery.getQueryObjectEXT(
            gpuTimer,
            extTimerQuery.QUERY_RESULT_EXT
          ) /
          (1000 * 1000);
        extTimerQuery.deleteQueryEXT(gpuTimer);
        this.fire(
          new Event("gpu-timing-frame", {
    ...
    if (this.listens("gpu-timing-layer")) {
      ...
      setTimeout(() => {
        const renderedLayerTimes =
          this.painter.queryGpuTimers(frameLayerQueries);

        this.fire(
          new Event("gpu-timing-layer", {
