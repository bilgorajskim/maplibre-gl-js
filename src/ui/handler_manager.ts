...
class HandlerManager {
  ...
  _blockedByActive(
    activeHandlers: { [x: string]: Handler },
  ...
  handleEvent(e: InputEvent | RenderFrameEvent, eventName?: string) {
    ...
    const inputEvent =
      e.type === "renderFrame" ? undefined : (e as any as InputEvent);
    ...
    const mapTouches = eventTouches
      ? this._getMapTouches(eventTouches)
      : undefined;
    const points = mapTouches
      ? DOM.touchPos(this._el, mapTouches)
      : DOM.mousePos(this._el, e as any as MouseEvent);
  ...
  _updateMapTransform(
    ...
    let { panDelta, zoomDelta, bearingDelta, pitchDelta, around, pinchAround } =
      combinedResult;
  ...
  _fireEvents(
    newEventsInProgress: { [x: string]: any },
    deactivatedHandlers: any,
