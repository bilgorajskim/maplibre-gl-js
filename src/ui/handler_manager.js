...
class HandlerManager {
    ...
    handleEvent(e: InputEvent | RenderFrameEvent, eventName?: string) {
        ...
        const inputEvent =
            e.type === "renderFrame" ? undefined : ((e: any): InputEvent);
        ...
        const mapTouches = e.touches
            ? this._getMapTouches(((e: any): TouchEvent).touches)
            : undefined;
        const points = mapTouches
            ? DOM.touchPos(this._el, mapTouches)
        ...
        for (const { handlerName, handler, allowed } of this._handlers) {
            ...
            if (this._blockedByActive(activeHandlers, allowed, handlerName)) {
                ...
                if ((handler: any)[eventName || e.type]) {
                    data = (handler: any)[eventName || e.type](
                        e,
                        points,
                        mapTouches
                    );
        ...
        if (
            Object.keys(activeHandlers).length ||
            hasChange(mergedHandlerResult)
        ) {
    ...
    _updateMapTransform(
        combinedResult: any,
        combinedEventsInProgress: Object,
        deactivatedHandlers: Object
    ) {
        const map = this._map;
    ...
    _fireEvents(
        newEventsInProgress: { [string]: Object },
        deactivatedHandlers: Object,
        ...
        if (allowEndAnimation && (wasMoving || nowMoving) && !stillMoving) {
            ...
            if (inertialEase) {
                if (
                    shouldSnapToNorth(
                        inertialEase.bearing || this._map.getBearing()
                    )
                ) {
    ...
    _requestFrame() {
        ...
        return this._map._renderTaskQueue.add((timeStamp) => {
            ...
            this.handleEvent(
                new RenderFrameEvent("renderFrame", { timeStamp })
            );
            this._applyChanges();
        });
    }
